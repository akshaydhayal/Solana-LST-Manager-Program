...
pub fn process_instruction(program_id:&Pubkey, accounts:&[AccountInfo], instruction_data:&[u8])->ProgramResult{
    ...
    match ix{
        InstructionType::InitialiseLST { lst_manager_bump, lst_manager_vault_bump,lst_mint_bump,stake_registry_record_bump}=>{
            msg!("initialise LST ix called");
            initialise_lst(program_id, accounts, lst_manager_bump, lst_manager_vault_bump,lst_mint_bump, stake_registry_record_bump)?;
        },
        ...
        InstructionType::StakeVaultSOL { lst_manager_bump, lst_manager_vault_bump,stake_acc_bump ,stake_registry_record_bump}=>{
            msg!("stake vault sol ix called");
            stake_vault_sol(program_id, accounts, lst_manager_bump, lst_manager_vault_bump,stake_acc_bump, stake_registry_record_bump)?;
        },
        ...
        InstructionType::UnstakeSOL {stake_acc_index, lst_manager_bump,lst_manager_vault_bump,epoch_withdraw_bump, stake_acc_bump ,split_stake_acc_bump,stake_registry_record_bump}=>{
            unstake_sol(program_id, accounts, stake_acc_index, lst_manager_bump, lst_manager_vault_bump, epoch_withdraw_bump, stake_acc_bump, split_stake_acc_bump, stake_registry_record_bump)?;
        }
    }
    Ok(())
}