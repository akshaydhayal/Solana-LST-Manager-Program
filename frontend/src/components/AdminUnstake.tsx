...
const AdminUnstake = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-800/90 via-gray-800/80 to-gray-800/90 backdrop-blur-sm rounded-xl border-2 border-orange-500/20 shadow-lg shadow-orange-500/10 p-5">
        <h3 className="text-lg font-semibold mb-3.5 flex items-center gap-1.5 bg-gradient-to-r from-orange-400 to-red-400 bg-clip-text text-transparent">
            <TrendingDown size={20} className="text-orange-400" />Unstake SOL from Validator for User's dSOL Burn Request
        </h3>

        <div className="space-y-3.5">
            <div>
                <label className="text-xs text-orange-300 mb-1.5 block">Select Stake Account</label>
                <select value={selectedStakeAccount} onChange={(e) => setSelectedStakeAccount(e.target.value)}
                        className="w-full bg-gradient-to-br from-gray-900/60 to-gray-900/40 border-2 border-orange-500/30 text-white rounded-lg px-3.5 py-2.5 focus:outline-none focus:border-orange-500 focus:shadow-lg focus:shadow-orange-500/20 transition-all cursor-pointer text-sm">
                ...
                <div className="bg-gray-900/70 border border-gray-700/50 rounded-lg p-3.5">
                    <div className="grid grid-cols-2 gap-3.5 text-xs">
                        <div>
                            ...
                            <div className="text-base font-semibold text-white">{(stakeAccounts[Number(selectedStakeAccount)-1].stakeAmount)/LAMPORTS_PER_SOL} SOL</div>
                        </div>
                        <div>
                            ...
                            <div className="text-base font-semibold text-white">{stakeAccounts[Number(selectedStakeAccount)-1].activatedEpoch}</div>
                        </div>
                    </div>
                </div>
            )}

            <div className="bg-orange-600/10 border border-orange-500/30 rounded-lg p-3.5">
            ...
            {epochWithdrawExists === false ? (
                <div className="bg-red-600/10 border border-red-500/30 rounded-lg p-3.5">
                    <div className="flex gap-1.5 text-xs">
                ...
                <div className="bg-green-600/10 border border-green-500/30 rounded-lg p-3.5 text-green-300 text-xs">Withdraw request detected for this epoch. Unstake is allowed.</div>
            )}

            <button onClick={unstakeSOLbySplitStake} className="w-full bg-gradient-to-r from-orange-600 via-red-600 to-pink-500 hover:from-orange-500 hover:via-red-500 hover:to-pink-400 hover:shadow-2xl hover:shadow-orange-500/50 py-3 rounded-lg text-base font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer text-white"
